...
FUNCTION jm_solve_astro,radeg,decdeg,xpixel,ypixel,XIORDER=xiorder,ETAORDER=etaorder,XTERMS=xterms,$
...
;  make_astr,astro, CRPIX=crpix, CRVAL=crval, CD=cd, EQUINOX=2000., RADECSYS='FK5', CTYPE=['RA---TAN','DEC--TAN']
  make_astr,astro, CRPIX=crpix, CRVAL=crval, CD=cd, CTYPE=['RA---TAN','DEC--TAN']

  ;; add NAXIS1/NAXIS2 tags if they do not exist
;  IF(N_ELEMENTS(naxis1) GT 0)THEN astro.naxis[0]=naxis1
;  IF(N_ELEMENTS(naxis2) GT 0)THEN astro.naxis[1]=naxis2
  astro=create_struct(temporary(astro), 'NAXIS1', naxis1)
  astro=create_struct(temporary(astro), 'NAXIS2', naxis2)
