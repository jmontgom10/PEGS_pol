...
; USEAGE:   tvim,a
;
;           tvim,a,scale=scale,range=range,xrange=xrange,xreverse=xreverse, $
;                yrange=yrange,yreverse=yreverse,aspect=aspect,title=title, $
;                xtitle=xtitle,ytitle=ytitle,noaxis=noaxis,interp=interp,   $
;                colors=colors,c_map=c_map,stitle=stitle,rmarg=rmarg,       $
;                clip=clip,labels=labels,pcharsize=pcharsize,               $
;                lcharsize=lcharsize,nbotclr=nbotclr,clevels=clevels,       $
;                nodata=nodata,rgb_nodata=rgb_nodata,barwidth=barwidth,     $
;                position=position,noframe=noframe,rct=rct
;
...
;
;  xreverse
;    if set x-axis increases to the left
;
;  yrange
;    array spanning entire y axis range. (default = [0,y_dimension-1])
;
;  yreverse
;    if set y-axis increases toward the bottom
;
...
; 19dec12: added XREVERSE and YREVERSE keywords
;-

pro tvim,a,scale=scale,range=range,xrange=xrange,xreverse=xreverse, $
         yrange=yrange,yreverse=yreverse,aspect=aspect,title=title, $
         xtitle=xtitle,ytitle=ytitle,noaxis=noaxis,interp=interp,   $
         colors=colors,c_map=c_map,stitle=stitle,rmarg=rmarg,       $
         clip=clip,labels=labels,pcharsize=pcharsize,               $
...
if iinodata(0) ne -1 then aa(iinodata)=9
...
if not keyword_set(xrange) then xrng=[0,nxm] $
                           else if keyword_set(xreverse) then xrng=[max(xrange), min(xrange)] $
                           else xrng=[min(xrange), max(xrange)]
if not keyword_set(yrange) then yrng=[0,nym] $
                           else if keyword_set(yreverse) then yrng=[max(yrange), min(yrange)] $
