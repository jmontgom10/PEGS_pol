...
PRO S3_ROTATE_IMAGE, event
  ...
  IF ~VALID_NUM(rotAngle) OR $
     ~VALID_NUM(xCen) OR $
     ~VALID_NUM(yCen) THEN BEGIN
    PRINT_TEXT2, event, 'The proposed rotation cannot be parsed.'
    RETURN
  ENDIF
